...
def prune_old(keep_days: int = 35) -> int:
    """Delete ranked rows older than *keep_days*.
...
def save_ranked_videos(
    ...
    """Save ranked videos for one platform across all time windows.

    Replaces any existing rows for the same *platform* + *scraped_date*
    combination (run-delete-then-insert within a single transaction).
    ...
    try:
        with SessionLocal() as session:
            with session.begin():
                # Remove old data for this platform + date combination
                session.execute(
                    delete(Video).where(
                ...
                for window_label, videos in windows.items():
                    for v in videos:
                        ...
                        total += 1
        logger.info("Saved %s data for %s: %d rows", platform, scraped_date, total)
    except SQLAlchemyError as e:
        logger.error("Failed to save %s data for %s: %s", platform, scraped_date, e)
    return total
...
def get_videos(
    ...
    try:
        ...
        logger.error("get_videos(%s, %s, %s): %s", platform, window, date, e)
        return []
...
def get_channel_stats(
    ...
    try:
        ...
        logger.error("get_channel_stats(%s, %s, %s): %s", platform, window, date, e)
        return []


def get_available_dates(platform: str) -> list[str]:
    """Get the list of dates that have data for a platform (newest first)."""
    SessionLocal = get_session()
    try:
        with SessionLocal() as session:
            stmt = (
        ...
        logger.error("get_available_dates(%s): %s", platform, e)
        return []
