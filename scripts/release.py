...
def main() -> None:
    ...
    tag = f"v{__version__}"
    dist = ROOT / "dist" / "HeatMap"

    if not args.no_build:
        run([sys.executable, "-m", "PyInstaller", "--noconfirm", "HeatMap.spec"], cwd=ROOT)

    if not (dist / "HeatMap.exe").exists():
        sys.exit(f"build output not found: {dist / 'HeatMap.exe'}")

    # Zip the *contents* of dist/HeatMap so the archive root holds HeatMap.exe
