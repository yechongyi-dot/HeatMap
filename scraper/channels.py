...
def _scrape_channel_batch(channels: list[dict], known_ids: set[str]) -> dict[str, dict]:
        ...
        for i, ch in enumerate(channels):
            ...
            try:
                result = yt.search_channel(
                    ch["channel_id"], "投資", max_results=VIDEOS_PER_CHANNEL,
                )
                for v in result.videos:
                    vid = v.video_id
                    if vid in known_ids or vid in out:
                        continue
                    if not is_investment_related(v.title):
                        continue
...
def scrape_channels_from_keyword_results(keyword_videos: list[dict]) -> list[dict]:
    ...
    with ThreadPoolExecutor(max_workers=workers) as pool:
        futures = [pool.submit(_scrape_channel_batch, b, known_ids) for b in batches if b]
...
def scrape_all_channels(keyword_videos: list[dict]) -> list[dict]:
    """Convenience wrapper for runner compatibility."""
    return scrape_channels_from_keyword_results(keyword_videos)
