...
from .config import OFFICIAL_CHANNELS, OFFICIAL_WORKERS
...
def _parse_feed(name: str, channel_id: str, needs_filter: bool = False) -> list[dict]:
    ...
    *needs_filter* applies the finance keyword check (used for broad-news
    ...
    for e in root.findall("a:entry", _NS):
        ...
        if needs_filter and not (is_investment_related(title) or is_investment_related(desc)):
            continue
...
def scrape_all() -> list[dict]:
    ...
    workers = max(1, min(OFFICIAL_WORKERS, len(OFFICIAL_CHANNELS)))
    all_videos: dict[str, dict] = {}
    with ThreadPoolExecutor(max_workers=workers) as pool:
        futures = [
            pool.submit(_parse_feed, ch["name"], ch["channel_id"], ch.get("filter", False))
    ...
    logger.info("Official total unique: %d (%d channels)", len(videos), len(OFFICIAL_CHANNELS))
    return videos
