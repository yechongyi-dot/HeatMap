"""YouTube scraper — parallel keyword search + channel deep-scrape.
...
from .config import (
    KEYWORDS, RESULTS_PER_KEYWORD, REQUEST_DELAY, REQUEST_JITTER, YOUTUBE_WORKERS,
)
        ...
        for keyword in keywords:
            for attempt in range(1, MAX_RETRIES + 1):
                try:
                    ...
                    for v in r.videos:
                        if v.video_id not in results and is_investment_related(v.title):
...
def _parallel_search(
    ...
    with ThreadPoolExecutor(max_workers=workers) as pool:
        futures = {
            pool.submit(_search_batch, batch, upload_date, idx): idx
...
def scrape_all_keywords() -> list[dict]:
    ...
    _merge(_parallel_search(KEYWORDS, "today", YOUTUBE_WORKERS, "Today"), "Phase A")

    logger.info("Phase B: upload_date=this_week (%d keywords)", len(KEYWORDS[::2]))
    _merge(_parallel_search(KEYWORDS[::2], "this_week", YOUTUBE_WORKERS, "Week"), "Phase B")

    logger.info("Phase C: upload_date=this_month (%d keywords)", len(KEYWORDS[::3]))
    _merge(_parallel_search(KEYWORDS[::3], "this_month", YOUTUBE_WORKERS, "Month"), "Phase C")
...
def scrape_all() -> list[dict]:
    ...
    logger.info("=" * 50)
    logger.info("Phase 1: Parallel keyword search (today + this_week)")
    keyword_videos = scrape_all_keywords()

    logger.info("=" * 50)
    logger.info("Phase 2: Channel deep-scrape")
    channel_videos = scrape_channels_from_keyword_results(keyword_videos)
