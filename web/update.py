...
def start_update_job(download_url: str) -> str:
    ...
    def _work() -> None:
        try:
            ...
            if new_root is None:
                raise RuntimeError("更新包结构异常：未找到 HeatMap.exe")
...
def _find_app_root(base: Path) -> Path | None:
    """Locate the folder containing HeatMap.exe (zip root or one level down)."""
    if (base / "HeatMap.exe").exists():
        return base
    for sub in base.iterdir():
        if sub.is_dir() and (sub / "HeatMap.exe").exists():
            return sub
    return None
...
def _write_updater_script(new_root: Path, target: Path, tmp: Path) -> Path:
    ...
    exe = target / "HeatMap.exe"
    script = (
        "@echo off\r\n"
        "chcp 65001 >nul\r\n"
        "echo ============================================\r\n"
        "echo   HeatMap is updating, please wait...\r\n"
        "echo ============================================\r\n"
        ":waitloop\r\n"
        'tasklist /fi "imagename eq HeatMap.exe" 2>nul | find /i "HeatMap.exe" >nul\r\n'
        "if not errorlevel 1 (\r\n"
