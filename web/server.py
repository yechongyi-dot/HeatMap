...
app = FastAPI(title="HeatMap - 日本投資動画ランキング")
...
def api_videos(
    ...
    """Get ranked videos for a platform + time window + date."""
    videos = store.get_videos(platform=platform, window=window, date=date, limit=limit)
...
def api_channels(
    ...
    channels = store.get_channel_stats(platform=platform, window=window, date=date)
...
def _run_scrape(job_id: str, q: queue.Queue) -> None:
    """Run full scrape in background thread, pushing progress to queue."""
    global _scrape_running
    try:
        from scraper.runner import run_all
        q.put({"type": "start", "message": "正在采集 YouTube 和 NicoNico 数据..."})

        def _on_progress(ev: dict) -> None:
            # Forward per-platform milestone events to the SSE stream
            q.put({"type": "platform", **ev})

        results = run_all(progress=_on_progress)
        ok_count = sum(1 for v in results.values() if v)
        total = len(results)
        q.put({
            "type": "complete",
            "ok": ok_count == total,
            "message": f"采集完成: {ok_count}/{total} 个平台成功",
            "results": results,
        })
    except Exception as e:
...
def api_scrape_start():
    """Start background scraping, return job_id for SSE progress."""
    ...
    t = threading.Thread(target=_run_scrape, args=(job_id, q), daemon=True)
    t.start()
